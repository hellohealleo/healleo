...
export function Dashboard({todayLog,weekLogs,weekDays,waterGoal,insights,profile,switchTab,labResults,aiMemory,state,update}){
  ...
  const proLabels = { doctor: "Dr. Healleo", nutritionist: "Dietitian", trainer: "Trainer", therapist: "Therapist" };
  ...
  return(<div className="fade-up"><h2 style={S.h2}>Good {new Date().getHours()<12?"morning":new Date().getHours()<17?"afternoon":"evening"}{profile.name?`, ${profile.name}`:""}</h2>
          ...
          <div key={i} className="card" style={{
            ...
            <div style={{display:"flex",gap:10}}>
              ...
              <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={() => switchTab(card.action)}>
                ...
                {card.crossAgent && <span style={{fontSize:11,color:"var(--dim)",marginTop:4,display:"inline-block"}}><Icon name="share" size={12}/> Shared with {proLabels[card.crossAgent]}</span>}
              </div>
    ...
    {showLabFlags&&<div style={{...S.card,marginTop:10,padding:12,borderLeft:"3px solid var(--danger)",background:"rgba(184,84,84,0.06)",display:"flex",alignItems:"flex-start",gap:8}}><div style={{flex:1,cursor:"pointer"}} onClick={()=>switchTab("labs")}><div style={{fontSize:15,fontWeight:600,color:"var(--danger)"}}><Icon name="warning" size={14}/> {recentFlags.length} abnormal lab value{recentFlags.length>1?"s":""}</div><div style={{fontSize:14,color:"var(--dim)",marginTop:2}}>{recentFlags.map(f=>`${f.name}: ${f.value} [${f.flag}]`).join(" · ")}</div></div>{dismissBtn("lab-flags")}</div>}
    {showAiInsight&&<div style={{...S.card,marginTop:10,padding:12,borderLeft:"3px solid var(--accent3)",display:"flex",alignItems:"flex-start",gap:8}}><div style={{flex:1,cursor:"pointer"}} onClick={()=>switchTab("timeline")}><div style={{fontSize:15,color:"var(--dim)"}}><strong><Icon name="memory" size={14}/> Latest insight:</strong> {aiMemory[aiMemory.length-1].insight.slice(0,100)}...</div></div>{dismissBtn(`ai-insight-${aiMemory.length}`)}</div>}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(130px, 1fr))",gap:8,marginTop:10}}>{[{l:"Water",v:`${todayLog.water} oz`,max:waterGoal,cur:todayLog.water,c:"var(--accent3)",i:"water"},{l:"Calories",v:todayLog.calories||"—",max:cg,cur:todayLog.calories,c:"var(--accent4)",i:"fire"},{l:"Protein",v:todayLog.protein?`${todayLog.protein}g`:"—",max:pg,cur:todayLog.protein,c:"var(--accent)",i:"protein"},{l:"Sleep",v:todayLog.sleep?`${todayLog.sleep}hr`:"—",max:8,cur:todayLog.sleep,c:"var(--accent2)",i:"sleep"}].map((s,idx)=><div key={idx} className="card" style={{...S.card,textAlign:"center",padding:12}}><div style={{position:"relative",display:"inline-block"}}><RingProgress value={s.cur} max={s.max} color={s.c} size={44} stroke={4}/><span style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)"}}><Icon name={s.i} size={16}/></span></div><div style={{marginTop:4,fontFamily:"var(--display)",fontSize:15,fontWeight:500}}>{s.v}</div><div style={{fontSize:15,color:"var(--dim)"}}>{s.l}</div></div>)}</div>
