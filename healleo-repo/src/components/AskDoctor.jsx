...
export function AskDoctor({state,update}) {
  ...
  return(<div className="fade-up">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div><h2 style={S.h2}><Icon name="doctor" size={32} style={{marginRight:6}}/> Dr. Healleo</h2><p style={{fontSize:15,color:"var(--dim)",marginTop:2}}>Your data, {dataPoints} points deep</p></div>
    ...
    {dataPoints>0&&<div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
      {state.labResults?.length>0&&<span style={{fontSize:16,padding:"3px 8px",background:"rgba(107,90,36,0.1)",borderRadius:10,color:"var(--accent)"}}><Icon name="labs" size={14}/> {state.labResults.length} lab reports</span>}
      {state.symptomSessions?.length>0&&<span style={{fontSize:16,padding:"3px 8px",background:"rgba(0,190,214,0.1)",borderRadius:10,color:"var(--accent4)"}}><Icon name="search" size={14}/> {state.symptomSessions.length} symptom checks</span>}
      {state.aiMemory?.length>0&&<span style={{fontSize:16,padding:"3px 8px",background:"rgba(179,148,167,0.1)",borderRadius:10,color:"var(--accent3)"}}><Icon name="memory" size={14}/> {state.aiMemory.length} learned insights</span>}
    </div>}
    <div style={{...S.card,marginTop:12,padding:0,minHeight:400,maxHeight:"62vh",display:"flex",flexDirection:"column"}}>
      <div style={{flex:1,overflow:"auto",padding:16}}>
        {msgs.length===0&&!loading&&<div style={{textAlign:"center",padding:"24px 10px"}}><div style={{marginBottom:10}}><Icon name="doctor" size={80}/></div><p style={{fontFamily:"var(--display)",fontSize:16,fontWeight:500}}>Hey — I've been going through your chart.</p><p style={{fontSize:15,color:"var(--dim)",margin:"6px 0 16px",lineHeight:1.5}}>I have your labs, symptoms, meds, lifestyle data — the whole picture. Ask me anything. The more you share, the better I get at connecting the dots for you.</p>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>{STARTERS.map(s=><button key={s} onClick={()=>send(s)} style={{padding:"10px 14px",background:"var(--bg)",border:"1px solid var(--muted)",borderRadius:10,fontSize:15,color:"var(--text)",cursor:"pointer",textAlign:"left",fontFamily:"var(--body)"}} onMouseEnter={e=>e.target.style.borderColor="var(--accent)"} onMouseLeave={e=>e.target.style.borderColor="var(--muted)"}><Icon name="chat" size={14}/> {s}</button>)}</div></div>}
    ...
    <div style={{marginTop:10,padding:"10px 12px",background:"rgba(196,167,98,0.08)",borderRadius:8,border:"1px solid rgba(196,167,98,0.15)"}}>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>{["PubMed/MEDLINE","OpenFDA Drug Labels","FDA Adverse Events","Web Search","AI Memory"].map(s=><span key={s} style={{fontSize:15,padding:"2px 6px",background:"var(--card)",borderRadius:6,color:"var(--dim)"}}>{s}</span>)}</div>
