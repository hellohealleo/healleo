...
export function generateTeamInsights(state) {
  ...
  if (recentFlags.length >= 3) {
    cards.push({ pro: "doctor", icon: "doctor", title: "We should talk about your labs", text: `You've got ${recentFlags.length} flagged values in the last month. A few abnormals happen — but ${recentFlags.length} together is worth a closer look. I can walk you through what they mean together.`, action: "ask", priority: "high" });
  }
  ...
  if (lastLabDate && daysSince(lastLabDate) > 90) {
    cards.push({ pro: "doctor", icon: "doctor", title: "It's been a while since your last labs", text: `${daysSince(lastLabDate)} days since your last results. If you've had bloodwork done recently, upload it — I can spot trends your doctor might not have time to explain.`, action: "labs", priority: "medium" });
  }
  ...
  if (recurringArea) {
    cards.push({ pro: "doctor", icon: "doctor", title: `Your ${recurringArea[0].toLowerCase()} keeps coming up`, text: `${recurringArea[1]} symptom checks in that area over the last 2 months. That's a pattern worth investigating — not ignoring. Let's talk through what might be going on.`, action: "ask", priority: "high" });
  }
  ...
  if (newMeds.length > 0) {
    cards.push({ pro: "doctor", icon: "doctor", title: `Checking in on ${newMeds[0].name}`, text: `You started this ${daysSince(newMeds[0].addedAt.slice(0, 10)) === 0 ? "today" : daysSince(newMeds[0].addedAt.slice(0, 10)) + " days ago"}. Any side effects? I can flag what to watch for and check interactions with your other meds.`, action: "ask", priority: "medium" });
  }
  ...
  if (calPrior > 0 && cal7 > 0 && cal7 < calPrior * 0.7) {
    ...
    cards.push({ pro: "nutritionist", icon: "nutrition", title: "Your calories dropped off a cliff", text: `Down ${drop}% from last week — ${Math.round(calPrior)} to ${Math.round(cal7)}. Could be intentional, could be life getting in the way. Either way, let's make sure you're still fueling what your body needs.`, action: "nutritionist", priority: "high" });
  }
  ...
  if (avgProtein7.length >= 3) {
    ...
    if (avgP < proteinTarget * 0.6) {
      cards.push({ pro: "nutritionist", icon: "nutrition", title: "Protein's running low", text: `You're averaging ${Math.round(avgP)}g — your target is closer to ${Math.round(proteinTarget)}g. That's a gap that'll show up in energy, recovery, and muscle. I've got easy fixes if you want them.`, action: "nutritionist", priority: "medium" });
    }
  }
  ...
  if (avgCal7.length >= 6 && cal7 > 0) {
    ...
    if (variance < cal7 * 0.15) {
      cards.push({ pro: "nutritionist", icon: "nutrition", title: "Your nutrition is dialed in", text: `6+ days logged, consistent calories, and you're actually hitting your targets. That's not luck — that's discipline. Keep this going and your body will show it.`, action: "nutritionist", priority: "positive" });
    }
  }
  ...
  if (exerciseDaysPrior >= 3 && exerciseDays7 === 0 && last7.length >= 5) {
    cards.push({ pro: "trainer", icon: "trainer", title: "Hey — haven't seen you move this week", text: `You were hitting ${exerciseDaysPrior} days last week. This week: zero. No guilt trip — life happens. But let's get at least one session in. Even 15 minutes counts. Want me to build you a quick one?`, action: "trainer", priority: "high" });
  }
  ...
  if (recentExerciseOnBadSleep.length >= 2) {
    cards.push({ pro: "trainer", icon: "trainer", title: "You're training on fumes", text: `${recentExerciseOnBadSleep.length} workout days this week on under 6 hours of sleep. I respect the dedication, but your body recovers during sleep — not during reps. You're getting diminishing returns and increasing injury risk.`, action: "trainer", priority: "high" });
  }

  // Great exercise streak
  if (exerciseDays7 >= 5) {
    cards.push({ pro: "trainer", icon: "trainer", title: "5+ days this week. Respect.", text: `${exerciseDays7} training days. That's the kind of consistency that compounds. Just make sure you're getting recovery too — your muscles grow on rest days, not workout days.`, action: "trainer", priority: "positive" });
  }
  ...
  if (exerciseDays7 >= 3) {
    ...
    if (types.length === 1) {
      cards.push({ pro: "trainer", icon: "trainer", title: `All ${types[0].toLowerCase()}, all the time`, text: `${exTypes[types[0]]} sessions of the same thing. I love the commitment, but your body adapts. Let's mix in something different — even once a week makes a difference for balanced fitness.`, action: "trainer", priority: "medium" });
    }
  }

  // Sedentary 7+ days (with active logging)
  if (exerciseDays7 === 0 && exerciseDaysPrior === 0 && last7.length >= 5) {
    cards.push({ pro: "trainer", icon: "trainer", title: "Two weeks with no movement logged", text: `Not judging — just noticing. Even a 10-minute walk changes your day. I can build you something so easy it feels like cheating. Want to start small?`, action: "trainer", priority: "medium" });
  }
  ...
  if (moodDays.length >= 3) {
    ...
    if (declining && recent3[recent3.length - 1].mood <= 2) {
      cards.push({ pro: "therapist", icon: "therapist", title: "I've noticed your mood dipping", text: `Three days trending down — ending at ${["terrible", "rough", "okay", "good", "great"][recent3[recent3.length - 1].mood]}. That's not a diagnosis, it's just a pattern worth paying attention to. Sometimes talking through it helps more than pushing through it.`, action: "therapist", priority: "high" });
    }
  }
  ...
  if (moodDays.length >= 3 && sleepDays.length >= 3) {
    ...
    if (avgMood <= 2 && avgSleep < 6.5 && !cards.find(c => c.pro === "therapist")) {
      cards.push({ pro: "therapist", icon: "therapist", title: "Sleep and mood — they're connected", text: `Low sleep (${avgSleep.toFixed(1)} hrs avg) and low mood tend to feed each other. It's not just about willpower — it's biochemistry. Let's talk about breaking the cycle from either end.`, action: "therapist", priority: "high" });
    }
  }
  ...
  if (recentDiagnosis.length > 0 && !cards.find(c => c.pro === "therapist")) {
    ...
    cards.push({ pro: "therapist", icon: "therapist", title: "Checking in after your recent news", text: `"${event.title}" showed up in your timeline ${daysSince(event.date)} days ago. New diagnoses and medication changes can stir up a lot — uncertainty, frustration, grief. That's all normal. I'm here if you want to process any of it.`, action: "therapist", priority: "medium" });
  }
  ...
  if (moodDays.length >= 5) {
    const avgMood = moodDays.reduce((s, l) => s + l.mood, 0) / moodDays.length;
    if (avgMood >= 3.5) {
      cards.push({ pro: "therapist", icon: "therapist", title: "You're in a good place right now", text: `Mood has been solid for ${moodDays.length} days. I just want to name that — it matters. Whatever you're doing is working. Let's bottle it.`, action: "therapist", priority: "positive" });
    }
  }
  ...
  if (cal7 > 0 && cal7 < weight * 9 && moodDays.length >= 3) {
    ...
    if (avgMood <= 2 && !cards.find(c => c.title.includes("calories"))) {
      cards.push({ pro: "nutritionist", icon: "nutrition", title: "Underfueling might be affecting your mood", text: `${Math.round(cal7)} cal/day is low for your body, and your mood's been down too. That's not always a coincidence — your brain runs on glucose. Let's make sure you're eating enough to feel like yourself.`, action: "nutritionist", priority: "high", crossAgent: "therapist" });
    }
  }

  // Exercising a lot but not eating enough (trainer + nutritionist)
  if (exerciseDays7 >= 4 && cal7 > 0 && cal7 < weight * 11) {
    cards.push({ pro: "trainer", icon: "trainer", title: "Training hard, eating light — careful", text: `${exerciseDays7} workout days but only ${Math.round(cal7)} cal/day. You're in a deficit that could cost you muscle and energy. Your nutritionist and I agree: fuel the work.`, action: "nutritionist", priority: "medium", crossAgent: "nutritionist" });
  }
